...
// ログインチェック
export async function checkMustResetPassword(admin) {
  if (!admin) return { ok: false, error: "NO_ADMIN" };

  if (admin.must_reset_password === true) {
    // ★ API を呼ばずに、lib のロジックを直接呼ぶ
    await requestPasswordResetLogic(admin.email);

    return {
      ok: false,
      error: "アカウントがロックされました。メールからパスワードリセットをおこなってください。",
