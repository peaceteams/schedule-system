...
export async function sendLoginNotification(email, sessionId, ip, geo, ua) {
  const invalidateUrl =
    `${process.env.BASE_URL}/api/account/invalidate-session?session=${sessionId}`;
  ...
  const html = `
    ...
    <p><strong>IP:</strong> ${ip || "取得不可"}</p>
    <p><strong>地域:</strong> ${geo?.country || "不明"} / ${geo?.region || ""}</p>
    <p><strong>デバイス:</strong> ${ua || "不明"}</p>
